...
function limparCatalogosAntigos() {
  try {
    ...
    console.log(`Encontrados ${arquivos.length} catálogos. Limpando antigos...`);
...
async function executarScraperAutomatico() {
  if (scraperRodando) {
    console.log("Scraper já está em execução.");
    return;
  }
...
app.get("/debug", (req, res) => {
  res.json({
    cwd: process.cwd(),
    existeProdutosJson: fs.existsSync(
      path.resolve(process.cwd(), "produtos.json")
    ),
    existePastaImagens: fs.existsSync(
      path.resolve(process.cwd(), "imagens")
    ),
    totalImagens: fs.existsSync(
      path.resolve(process.cwd(), "imagens")
    )
      ? fs.readdirSync(
          path.resolve(process.cwd(), "imagens")
...
app.post("/scraper/catalogo", async (req, res) => {
  ...
  try {
    ...
    res.json({
      mensagem: "Scraper e catálogo executados com sucesso.",
      total: produtos.length,
      arquivo: pdf.nomeArquivo,
      download: `/catalogo/download/${pdf.nomeArquivo}`
    });
    ...
    console.error("Erro scraper/catalogo:", error);
...
app.get("/catalogo/download", (req, res) => {
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="${ultimoCatalogoGerado}"`
  );
...
cron.schedule("0 6 * * *", async () => {
  console.log("Executando scraper das 06:00");
  await executarScraperAutomatico();
});

cron.schedule("0 18 * * *", async () => {
  console.log("Executando scraper das 18:00");
  await executarScraperAutomatico();
});
